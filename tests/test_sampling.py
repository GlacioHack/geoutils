...
class TestSubsampling:
    ...
    def test_subsample(self, array: np.ndarray) -> None:
        """
        Test gu.georaster.subsample_array.
        """
        ...
        for npts in np.arange(2, np.size(array)):
            random_values = gu.georaster.subsample_array(array, subsample=npts)
        ...
        random_values = gu.georaster.subsample_array(array, subsample=np.size(array) + 3)
        assert np.all(np.sort(random_values) == array[~array.mask])

        # Test if subsample = 1 => return all valid values
        random_values = gu.georaster.subsample_array(array, subsample=1)
        assert np.all(np.sort(random_values) == array[~array.mask])

        # Test if subsample < 1
        random_values = gu.georaster.subsample_array(array, subsample=0.5)
        assert np.size(random_values) == int(np.size(array) * 0.5)

        # Test with optional argument return_indices
        indices = gu.georaster.subsample_array(array, subsample=0.3, return_indices=True)
        ...
        sub42 = gu.georaster.subsample_array(array, subsample=10, random_state=42)
        # Check by passing a generator directly
        random_gen = np.random.RandomState(np.random.MT19937(np.random.SeedSequence(42)))
        sub42_gen = gu.georaster.subsample_array(array, subsample=10, random_state=random_gen)
