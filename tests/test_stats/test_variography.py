...
class TestVariogramStorage:
    ...
    def test_importing_geoutils_does_not_load_variogram_backends(self) -> None:
        ...
        # 1/ Build a fresh Python process that imports only GeoUtils
        code = (
            "import sys\n"
            "import geoutils\n"
            "assert not {'skgstat', 'gstools', 'gpytorch', 'torch'}.intersection(sys.modules)\n"
        )
        # 2/ Check inside that process that none of the optional packages was loaded
    ...
    def test_variogram_is_small_immutable_and_serializable(self) -> None:
        ...
        # 1/ Create a complete measured and fitted result from small arrays and plain model details
        result = gu.Variogram(
        ...
        # 2/ Check that callers cannot change stored distance values in place
        with pytest.raises(ValueError, match="read-only"):
            result.lags[0] = 3

        # 3/ Convert through JSON and check the restored model and optional error values
        ...
        # 4/ Check that Xarray contains every per-distance value
    ...
    def test_from_pairs_does_not_retain_pair_data(self) -> None:
        """Checks that from_pairs() keeps per-distance results and releases individual pairs."""

        # 1/ Create twenty pairs with a constant endpoint difference of two
        ...
        # 2/ Reduce the pairs into four log-spaced distance bins
        result = gu.Variogram.from_pairs(pairs, estimator="matheron", bins="log", n_lags=4)

        # 3/ Check the bin count, total pair count, semivariance, and released source object
    ...
    def test_from_pairs_uses_sampled_distance_limits_for_stable_bins(self) -> None:
        ...
        # 1/ Store requested limits that extend beyond the distances drawn in this pair sample
        pairs = xr.Dataset(
            {
        ...
        # 2/ Build three log-spaced bins from the stored limits
        result = gu.Variogram.from_pairs(pairs, bins="log", n_lags=3)

        # 3/ Check both outer edges and that every sampled pair enters one bin
...
class TestVariogramEstimation:
    ...
    def test_object_variogram_aggregates_runs_and_fits_summed_model(self) -> None:
        ...
        # 1/ Create a smooth raster with variation in both grid directions
        y, x = np.mgrid[:35, :35]
        raster = gu.Raster.from_array(
            np.sin(x / 4) + 0.5 * np.cos(y / 10), from_origin(0, 35, 2, 2), 32633, nodata=None
        )
        # 2/ Combine three pair samples and fit Gaussian plus spherical components
        ...
        # 3/ Check both fitted components, sampling errors, released pairs, and run count
    ...
    def test_variogram_repetitions_match_independent_samples(self, n_runs: int) -> None:
        ...
        # 1/ Reproduce each pair sample separately with fixed distance bin edges
        y, x = np.mgrid[:20, :20]
        ...
        # 2/ Check the public result against the mean values and summed counts from separate samples
        ...
        # 3/ Check that sampling error is absent for one run and follows the usual standard error for repeated runs
        if n_runs == 1:
    ...
    def test_fit_accepts_short_names_and_skgstat_model_functions(self) -> None:
        ...
        # 1/ Create exact Gaussian values with the SciKit-GStat function
        ...
        # 2/ Fit one function and one short string name as a summed model
        fitted = empirical.fit([skgstat.models.gaussian, "Sph"])

        # 3/ Check the standard component names stored in order
    ...
    def test_pointcloud_variogram_and_advanced_pairs_share_api(self) -> None:
        ...
        # 1/ Create point values that vary smoothly in both coordinate directions
        y, x = np.mgrid[:18, :18]
        pointcloud = gu.PointCloud.from_xyz(x.ravel(), y.ravel(), (np.sin(x / 3) + np.cos(y / 5)).ravel(), crs=32633)

        # 2/ Request either the individual pairs or six reduced distance bins
        ...
        # 3/ Check the requested sizes and that no model is fitted by default
...
class TestVariogramConversion:
    ...
    def test_model_evaluation_and_gpytorch_parameters(self) -> None:
        """Checks Gaussian model evaluation and the matching plain GPyTorch parameters."""

        # 1/ Create a Gaussian model with no measured values on the first two coordinate columns
        ...
        # 2/ Check the converted length scale and model values at zero distance
    ...
    def test_product_model_multiplies_covariances(self) -> None:
        ...
        # 1/ Combine spatial and temporal models that use different coordinate columns
        ...
        # 2/ Check the combined sill, zero-distance values, coordinate columns, and observation noise
    ...
    def test_skgstat_estimation_can_discard_or_retain_backend(self) -> None:
        """Checks that direct SciKit-GStat estimation keeps its source object only when requested."""

        # 1/ Estimate the same coordinate values with and without keeping the SciKit-GStat object
        ...
        # 2/ Check both storage choices and the method that releases a kept object
        assert result.backend_object is None
    ...
    def test_gstools_conversion_matches_skgstat(self, model_name: str, smoothness: float | None) -> None:
        ...
        # 1/ Build one model with no measured values and convert it to GSTools
        ...
        # 2/ Calculate expected semivariances with the matching SciKit-GStat function
        ...
        # 3/ Check the GSTools model values and unchanged coordinate column selection
    ...
    def test_gstools_conversion_preserves_sum_and_common_active_dims(self) -> None:
        """Checks that summed GSTools models share coordinate columns and one parent nugget."""

        # 1/ Sum two models that use the same two coordinate columns
        ...
        # 2/ Check the shared columns, summed variance, and parent nugget
    ...
    def test_gstools_rejects_component_specific_dimensions(self) -> None:
        ...
        # 1/ Combine spatial and temporal models that select different columns
        ...
        # 2/ Check the clear error for this unsupported conversion
