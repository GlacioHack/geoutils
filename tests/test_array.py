...
class TestArray:
    ...
    def test_get_array_and_mask(self,
        ...
        if not check_should_pass:
            with pytest.raises(ValueError, match="Invalid array shape given"):
                gu.georaster.get_array_and_mask(array, check_shape=True)
        ...
        arr, _ = gu.georaster.get_array_and_mask(array, copy=True, check_shape=True)
        ...
        with warnings.catch_warnings(record=True) as caught_warnings:
            ...
            arr_view, mask = gu.georaster.get_array_and_mask(array, copy=False)
    ...
    def test_get_valid_extent(self) -> None:
        ...
        # For the array
        assert (0, 4, 0, 4) == gu.georaster.get_valid_extent(arr)
        # For the masked-array
        assert (0, 4, 0, 4) == gu.georaster.get_valid_extent(mask_ma)

        # 1/ First column:
        # If we mask it in the masked array
        mask_ma[0, :] = np.ma.masked
        assert (1, 4, 0, 4) == gu.georaster.get_valid_extent(mask_ma)

        # If we changed the array to NaNs
        arr[0, :] = np.nan
        assert (1, 4, 0, 4) == gu.georaster.get_valid_extent(arr)
        ...
        # If we mask it in the masked array
        mask_ma[:, 0] = np.ma.masked
        assert (0, 4, 1, 4) == gu.georaster.get_valid_extent(mask_ma)

        # If we changed the array to NaNs
        arr[:, 0] = np.nan
        assert (0, 4, 1, 4) == gu.georaster.get_valid_extent(arr)
        mask_ma.data[:, 0] = np.nan
        mask_ma.mask = False
        assert (0, 4, 1, 4) == gu.georaster.get_valid_extent(mask_ma)
        ...
        assert (0, 3, 0, 4) == gu.georaster.get_valid_extent(mask_ma)

        # If we changed the array to NaNs
        arr[-1, :] = np.nan
        assert (0, 3, 0, 4) == gu.georaster.get_valid_extent(arr)
        ...
        # If we mask it in the masked array
        mask_ma[:, -1] = np.ma.masked
        assert (0, 4, 0, 3) == gu.georaster.get_valid_extent(mask_ma)

        # If we changed the array to NaNs
        arr[:, -1] = np.nan
        assert (0, 4, 0, 3) == gu.georaster.get_valid_extent(arr)
        mask_ma.data[:, -1] = np.nan
        mask_ma.mask = False
        assert (0, 4, 0, 3) == gu.georaster.get_valid_extent(mask_ma)
    ...
    def test_get_xy_rotated(self) -> None:
        ...
        xx360, yy360 = gu.georaster.get_xy_rotated(r1, along_track_angle=360)
        assert np.allclose(xx360, xx)
        assert np.allclose(yy360, yy)

        # Test that the values make sense for 45 degrees
        xx45, yy45 = gu.georaster.get_xy_rotated(r1, along_track_angle=45)
        ...
        xx, yy = gu.georaster.get_xy_rotated(r1, along_track_angle=90)
