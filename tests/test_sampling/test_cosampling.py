...
class TestRasterPointModes:
    ...
    def test_grid_points(self, caller: str, accessor: bool, explicit_at: bool) -> None:
        ...
        # Check the means, added point values, and common mask against direct arrays
    ...
    def test_resample_raster(self, caller: str, method: str) -> None:
        ...
        # Create a sloping raster whose bilinear values can be calculated exactly
        ...
        # Read the raster at point locations through either public object
        first, second = (raster, points) if caller == "raster" else (points, raster)
    ...
    def test_grid_crs_alignment(self) -> None:
        ...
        # Express points from known raster cells in a different coordinate system
        raster = _raster(np.arange(20, dtype=float).reshape(4, 5))
        points = raster.to_pointcloud().reproject(crs=4326)

        # Require the caller to allow coordinate conversion
        ...
        # Check that converting back places every point in its original cell
    ...
    def test_dask_gridding_chunks(self, chunks: tuple[int, int], caller: str, tmp_path: Path) -> None:
        """Checks that lazy point gridding keeps values and seeded sample locations across chunk sizes."""

        # Place one point at each grid cell so the expected nearest-neighbor grid is exact
    ...
    def test_mode_validation_and_reduction_placeholder(self) -> None:
        """Checks that unclear output locations, conflicting modes, and window reduction raise clear errors."""
        ...
        # Check that unclear and conflicting output choices raise useful errors
        ...
        # Check that the reserved point window option reports that it is not implemented yet
...
class TestRasterSupport:
    ...
    def test_joint_validity_and_auxiliary(self, accessor: bool) -> None:
        ...
        # Check the output grid, common mask, and documented band order
        ...
        # Check every kept value directly, including valid zeros
    ...
    def test_selected_bands_and_auxiliary_owners(self) -> None:
        ...
        # Tie each plain added array to the grid whose cells it follows
    ...
    def test_masked_integers_and_boolean_masks(self, lazy: bool, raster_mask: bool) -> None:
        ...
        # Give each input and mask a different missing or false cell
        ...
        # Run with eager or lazy primary rasters and either array or raster masks
        source = first
        if lazy:
        ...
        # Check the common mask and values against the original arrays
    ...
    def test_dask_output_and_chunk_independence(self, subsample: int | float) -> None:
        ...
        # Store the two rasters with different Dask chunk layouts
        da = pytest.importorskip("dask.array")
        ...
        # Check laziness, chosen cells, and values against the eager result
...
class TestPointSupport:
    ...
    def test_mixed_inputs(self, caller: str, accessor: bool) -> None:
        """Checks that a raster and point comparison returns point locations through either calling interface."""

        # Give the point and raster inputs a missing value at different locations
        ...
        # Keep point output while calling through either spatial object or accessor
    ...
    def test_point_auxiliaries_and_support_labels(self, at: str | None) -> None:
        ...
        # Use duplicate row labels and 3D points so lost point information is visible
        ...
        # Choose either point input as the output locations
        selected_at = second if at == "explicit" else at
    ...
    def test_explicit_point_support_for_rasters(self) -> None:
        ...
        # Choose point locations that fall at known cells in both rasters
        ...
        # Check both raster values in the returned point columns
    ...
    def test_vector_mask(self, mask_mode: str) -> None:
        ...
        # Place two points inside a vector polygon and two outside
        raster = _raster(np.arange(30, dtype=float).reshape(5, 6))
        rows, columns = np.array([0, 1, 3, 4]), np.array([1, 2, 4, 5])
        x, y = raster.ij2xy(rows, columns)
        ...
        # Check the kept point labels and values for either mask direction
    ...
    def test_selected_band_validity(self, accessor: bool) -> None:
        ...
        # Put missing cells at different point locations in each raster band
        ...
        # Check that a point missing only from the unused first band remains
    ...
    def test_subsampling_and_independent_storage(self) -> None:
        """Checks that subsampling keeps point order and returns data independent of its inputs."""
