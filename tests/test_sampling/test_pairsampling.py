...
class TestRasterPairSampling:
    ...
    def test_raster_loglag_strategies_return_pair_dataset(self, raster: gu.Raster, strategy: str) -> None:
        ...
        # Draw raster pairs with one strategy and fixed distance limits
        ...
        # Check the common Xarray layout and that every pair follows the distance and missing data rules
    ...
    def test_dask_raster_pair_sampling_keeps_source_lazy(self) -> None:
        ...
        # Create a lazy raster with several chunks
        ...
        # Draw a fixed number of pairs through the Xarray accessor
        pairs = raster.rst.pairsample(n_pairs=250, random_state=42)

        # Check the result size and that the complete source is still lazy
...
class TestPointPairSampling:
    ...
    def test_pointcloud_random_pairs_honor_mask_indexes_and_dtypes(self) -> None:
        """Checks that independent point pairs keep row numbers after missing data and mask filtering."""
