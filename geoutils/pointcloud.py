...
def _grid_pointcloud(pc: gpd.GeoDataFrame,
                     grid_coords: tuple[NDArrayNum],
                     data_column_name: str = "b1",
                     resampling: Literal["nearest", "linear", "cubic"] = "linear",
                     dist_nodata_pixel: float = 1.) -> NDArrayNum:
    """
    ...
    grid_pc = gpd.GeoDataFrame(data={"placeholder": np.ones(len(xx.ravel()))},
    ...
    dist = np.sqrt(((pc.geometry.x.values[near["index_right"]] - grid_pc.geometry.x.values) / res_x) ** 2 +
