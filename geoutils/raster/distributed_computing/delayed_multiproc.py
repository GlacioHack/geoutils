...
def reproject_block(
    ...
    # Reproject the raster tile
    return raster_tile.reproject(ref=dst_raster_tile, resampling=resampling, silent=silent), dst_tile
...
def multiproc_reproject(
    ...
    dst_raster = Raster(output_file)

    dst_grid = compute_tiling(tile_size, dst_raster.shape, dst_raster.shape)

    for row in range(dst_grid.shape[0]):
        for col in range(dst_grid.shape[1]):
            dst_tile = dst_grid[row, col]
            task.append(
                cluster.launch_task(fun=reproject_block, args=[raster_unload, dst_raster, dst_tile, resampling, silent])
            )
    ...
    with rio.open(output_file, "r+") as dst:
        try:
            # Retrieve reprojection results
            for result in task:
                reprojected_tile, dst_tile = cluster.get_res(result)
