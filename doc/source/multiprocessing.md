...
- {func}`~geoutils.multiproc.map_overlap_multiproc_save`: Applies a function to raster tiles and **saves the output** as a {class}`geoutils.Raster`.
- {func}`~geoutils.multiproc.map_multiproc_collect`: Applies a function and **collects extracted data** from raster tiles into a list.
...
{class}`~geoutils.multiproc.MultiprocConfig` defines tiling and processing settings, such as chunk size, output file, driver, and computing cluster. It ensures that computations are performed **without loading the entire raster into memory**.
...
## {func}`~geoutils.multiproc.map_overlap_multiproc_save`: process and save large rasters
...
import scipy
import numpy as np
from geoutils.multiproc import map_overlap_multiproc_save
...
## {func}`~geoutils.multiproc.map_multiproc_collect`: extract and collect data from large rasters
...
```{code-cell} ipython3
from geoutils.multiproc import map_multiproc_collect
...
| Use case                                      | Function                                                               |
|-----------------------------------------------|------------------------------------------------------------------------|
| Apply processing and save results as a raster | {func}`~geoutils.multiproc.map_overlap_multiproc_save`                 |
| Extract statistics or features into a list    | {func}`~geoutils.multiproc.map_multiproc_collect`                         |
| Track tile locations with extracted data      | {func}`~geoutils.multiproc.map_multiproc_collect` with `return_tile=True` |
