...
This example demonstrates the reprojection of a raster using :meth:`~geoutils.raster.base.RasterBase.reproject`.
"""
...
rast1 = gu.open_raster(filename_rast1)
rast2 = gu.open_raster(filename_rast2)
...
rast1.rst.plot(cmap="Blues")
vect_bounds_rast2 = gu.VectorAccessor.from_bounds_projected(rast2)
vect_bounds_rast2.vct.plot(fc="none", ec="r", lw=2)

# %%
# **First option:** using the second raster as a reference to match, we reproject the first one. We simply have to pass the second raster
# as single argument to :meth:`~geoutils.raster.base.RasterBase.reproject`. See :ref:`core-match-ref` for more details.
#
# By default, a "bilinear" resampling algorithm is used. Any string or :class:`~rasterio.enums.Resampling` can be passed.

rast1_warped = rast1.rst.reproject(rast2)
rast1_warped
...
#   use :meth:`~geoutils.raster.base.RasterBase.set_nodata`.
#
# Now the shape and georeferencing should be the same as that of the second raster, shown above.

rast1_warped.rst.info()

# %%
# We can plot the two rasters next to one another

rast1_warped.rst.plot(ax="new", cmap="Reds")
rast2.rst.plot(ax="new", cmap="Blues")

# %%
# **Second option:** we can pass any georeferencing argument to :meth:`~geoutils.raster.base.RasterBase.reproject`, such as ``dst_size`` and ``dst_crs``, and will only
...
rast2.rst.set_nodata(0)
# Pass the desired georeferencing parameters
rast2_warped = rast2.rst.reproject(grid_size=(100, 100), crs=32645)
rast2_warped.rst.info()
